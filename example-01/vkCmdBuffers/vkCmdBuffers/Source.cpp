...
int main()
{
	...
	*/
	...
	Buffer StagingBuffer, VertexBuffer;
	...
	result = vkCreateBuffer(device, &vertexBufferInfo, nullptr, &StagingBuffer.buffer);
	...
	void *data;
	result = vkAllocateMemory(device, &memAlloc, nullptr, &StagingBuffer.memory);
	if (result == VK_SUCCESS) { std::cout << "Device Memory Allocation Success" << std::endl; }

	result = vkMapMemory(device, StagingBuffer.memory, 0, memAlloc.allocationSize, 0, &data);
	memcpy(data, Vertices.data(), memAlloc.allocationSize);
	vkUnmapMemory(device, StagingBuffer.memory);

	result = vkBindBufferMemory(device, StagingBuffer.buffer, StagingBuffer.memory, 0);
	...
	*/
	...
	memAlloc.allocationSize = memReqs.size;
	vkAllocateMemory(device, &memAlloc, nullptr, &VertexBuffer.memory);
	...
	vkCmdCopyBuffer(copyCommandBuffer, StagingBuffer.buffer, VertexBuffer.buffer, 1, &copyRegion);
